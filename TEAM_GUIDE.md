...
The training dataset `intelligence-service/training/sudoku-3m.csv` (approximately 511 MB) was exceeding GitHub's 100 MB file-size limit, which blocked all pushes.
...
SudokuAscent/
  ...
  intelligence-service/           Python ML / AI service
    ...
    training/
      ...
      sudoku-3m.csv               3M puzzle dataset (approximately 511 MB, stored via Git LFS)
...
| **Install Git LFS before cloning** | Run `git lfs install` once per machine. Without it, you will get pointer files instead of actual data. |
| **Never commit files over 100 MB without LFS** | GitHub will reject the push. If you need to track other large file types, add them with `git lfs track "*.extension"` |
| **Do not force-push shared branches** | Only force-push if you have coordinated with the team (like this LFS migration). |
